document.addEventListener('DOMContentLoaded', () => {
    ...
    studyPeriodInput.addEventListener('input', (e) => {
        studyPeriodValue.textContent = `${Number(e.target.value)} 年`;
    });

    partTimeHoursInput.addEventListener('input', (e) => {
        partTimeHoursValue.textContent = `${Number(e.target.value)} 小時`;
    });
    ...
    calculateBtn.addEventListener('click', () => {
        ...
        try {
            ...
            document.getElementById('studyPayment').textContent = `${formatHKD(mth_SP)}/月`;
            document.getElementById('gradPayment').textContent = `${formatHKD(mth_RP)}/月`;
    ...
    const drawRepaymentChart = (sp, mth_SP, mth_RP) => {
        ...
        const labels = [...Array(studyYears).fill('學習期間'), '寬限期', ...Array(10).fill(0).map((_, i) => `還款第 ${i+1} 年`)];
    ...
    const drawRefiChart = (mth_RP, mth_tot_refi_first60, mth_tot_refi_last60) => {
        ...
        refiChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: ['無再融資 (10年)', '有再融資 (第1-5年)', '有再融資 (第6-10年)'],
                datasets: [{ label: '每月還款', data: [mth_RP, mth_tot_refi_first60, mth_tot_refi_last60], backgroundColor: ['#333333', '#002e5d', '#002e5d'] }]
            },
            options: {
                responsive: true,
                maintainAspectRatio: false,
    ...
    const updateRefiDescription = (salary) => {
        ...
        const staticText = "畢業後，您將成為銀行的優質客戶。透過將您的SocioBridge部分貸款進行再融資，您可以節省大量資金。調整潛在的銀行利率以查看效果。 ";
        
        const dynamicText = `這假設您申請一筆為期5年的銀行貸款，金額為 ${formatHKD(bank_amt)} (即您預期月薪 ${formatHKD(salary)} 的12倍)。`;
    ...
    loadAllJSONData().then(() => {
        ...
        studyPeriodValue.textContent = `${Number(studyPeriodInput.value)} 年`;
        partTimeHoursValue.textContent = `${Number(partTimeHoursInput.value)} 小時`;
    });
});
